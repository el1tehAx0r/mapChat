...
export class FramePicker extends Component {
  static propTypes = {
 ...
 onStartShouldSetPanResponder: (evt, gestureState) => {true},
  ...
  _handleTimeMarkerMove(e,gestureState){
    ...
    if (rightPos - timePos<= 50 && timePos-leftPos<=50) {
      return;
    }
    this._callOnChange();
    Animated.event([
  ...
  renderRightSection() {
    ...
    return (
      <Animated.View
        style={[styles.container, styles.rightCorner, { left: layoutWidth+50 }, {
          transform: [{
  ...
  renderTimeMarker() {
    ...
    return (
      <Animated.View
        style={[styles.container, styles.rightCorner, { right: -layoutWidth+50  }, {
          transform: [{
