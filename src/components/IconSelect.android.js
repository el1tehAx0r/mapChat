...
export class IconSelect extends Component {
 ...
 onStartShouldSetPanResponder: (evt, gestureState) => {true},
  ...
  _handleTimeMarkerMove(e,gestureState){
    ...
    calculateCornerResult(duration, this._leftCornerPos, layoutWidth)
    console.log(layoutWidth)
    console.log(width)
    console.log(duration*1000)
    this._currentTime=calculateCornerResult(duration,this._timeMarkerPos-180,layoutWidth,true)*1000
    this.props.timeMarkerChanged(this._currentTime)
    console.log(this._currentTime,'SDKFL')
    const rightPos = layoutWidth - Math.abs(this._rightCornerPos);
    const moveTime = gestureState.dx ;
    this._callOnChange();
    Animated.event([
